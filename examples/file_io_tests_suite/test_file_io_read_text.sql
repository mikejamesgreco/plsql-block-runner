...
BEGIN
  ...
  -- Setup: create a text file using UTL_FILE (so READ_TEXT is truly being tested)
  declare
    l_f utl_file.file_type;
  begin
    l_f := utl_file.fopen('XX_DBADIR_SECURE', 'ut_read_text_src.txt', 'w', 32767);
    utl_file.put_line(l_f, 'Line 1');
    utl_file.put_line(l_f, 'Line 2');
    utl_file.fclose(l_f);
  end;
  ...
  -- Teardown
  BEGIN
    UTL_FILE.fremove('XX_DBADIR_SECURE','ut_read_text_src.txt');
  EXCEPTION
