...
BEGIN
  ...
  begin
    l_f := utl_file.fopen('XX_DBADIR_SECURE', 'ut_move_src.txt', 'w', 32767);
    utl_file.put_line(l_f, 'move me');
    utl_file.fclose(l_f);
  end;
  ...
  run_case(q'~{
    ...
    "src_name": "ut_move_src.txt",
    "dst_dir": "XX_DBADIR_SECURE",
    "dst_name": "ut_move_dst.txt",
    "overwrite": true
  }~');
  ...
  run_case(q'~{
    ...
    "name": "ut_move_src.txt"
  }~');
  
  run_case(q'~{
    "op": "GETATTR",
    "dir": "XX_DBADIR_SECURE",
    "name": "ut_move_dst.txt"
  }~');
